...
// source: geon/src/gpu/shaders/dispatch-args.wgsl
// helpers-sha256: 165cb4ebb9e35e86beab2561fee3a1f79d834c73f5b65aae6134ec1b4a163356
// wgsl-transpile sha256: 870c4f7c8eaa8cc229d18bcecc987c019bd1a7597bd8d446abb5e05c99b357cb
...
// wgsl-metrics: {"bytes":15512,"lines":326,"rtVec":0,"rtPoly":0,"rtAtomic":0,"rtNumeric":0,"fround":0,"hypot":0,"iife":0,"workgroupReductionInits":0,"flatWorkgroupArrays":0,"flatWorkgroupSlots":0,"staticBranchPrunes":0}
// generated: 2026-05-30T22:00:38.803Z
export default function _wgsl_module(rt) {
    ...
    entryInfo["buildBosonDispatchArgs"] = {"workgroupSize":[1,1,1],"phases":1,"globalLoop":true,"workgroupMemory":false,"flatWorkgroupArrays":0,"optimizedWorkgroupReductionInits":0};
    function __entry_0_buildBosonDispatchArgs(workgroups, bindings, domain, origin) {
        const [Wx, Wy, Wz] = workgroups;
        const Lx = 1, Ly = 1, Lz = 1;
        const _b_phCount = bindings.phCount;
        const _b_piCount = bindings.piCount;
        const _b_dispatchArgs = bindings.dispatchArgs;
        ...
        if (Gy === 1 && Gz === 1) {
            for (let __gx = Ox; __gx < Xn; __gx++) {
                {
                    const phN = ((MAX_PHOTONS) < (_b_phCount[0]) ? (MAX_PHOTONS) : (_b_phCount[0]));
                    const piN = ((PION_POOL_CAP) < (_b_piCount[0]) ? (PION_POOL_CAP) : (_b_piCount[0]));
                    const total = (((MAX_PHOTONS + PION_POOL_CAP)) < ((phN + piN)) ? ((MAX_PHOTONS + PION_POOL_CAP)) : ((phN + piN)));
                    let _inl_24_result;
                    _inl_24: {
                        _inl_24_result = ((1) < ((((phN + 63)) / 64)) ? ((((phN + 63)) / 64)) : (1));
                        break _inl_24;
                    }
                    _b_dispatchArgs[0] = _inl_24_result;
                    _b_dispatchArgs[1] = 1;
                    _b_dispatchArgs[2] = 1;
                    let _inl_25_result;
                    _inl_25: {
                        _inl_25_result = ((1) < ((((piN + 63)) / 64)) ? ((((piN + 63)) / 64)) : (1));
                        break _inl_25;
                    }
                    _b_dispatchArgs[3] = _inl_25_result;
                    _b_dispatchArgs[4] = 1;
                    _b_dispatchArgs[5] = 1;
                    let _inl_26_result;
                    _inl_26: {
                        _inl_26_result = ((1) < ((((total + 63)) / 64)) ? ((((total + 63)) / 64)) : (1));
                        break _inl_26;
                    }
                    _b_dispatchArgs[6] = _inl_26_result;
                    _b_dispatchArgs[7] = 1;
                    _b_dispatchArgs[8] = 1;
                    const _inl_27_n = (total * 6);
                    let _inl_27_result;
                    _inl_27: {
                        _inl_27_result = ((1) < ((((_inl_27_n + 63)) / 64)) ? ((((_inl_27_n + 63)) / 64)) : (1));
                        break _inl_27;
                    }
                    _b_dispatchArgs[9] = _inl_27_result;
                    _b_dispatchArgs[10] = 1;
                    _b_dispatchArgs[11] = 1;
                }
            }
        } else if (Gz === 1) {
            if (Ox === 0 && Oy === 0) {
    ...
    entry["buildBosonDispatchArgs"] = function ({ workgroups, bindings, domain, origin }) {
        return __entry_0_buildBosonDispatchArgs(workgroups, bindings, domain, origin);
    };

    const bind = function (bindings) {
        const bound = Object.create(null);
        bound["buildBosonDispatchArgs"] = function (workgroups, domain, origin) {
            return __entry_0_buildBosonDispatchArgs(workgroups, bindings, domain, origin);
        };
        return bound;
    };

    return { entry, bind, bindings: ["phCount","piCount","dispatchArgs"], entryInfo };
}
