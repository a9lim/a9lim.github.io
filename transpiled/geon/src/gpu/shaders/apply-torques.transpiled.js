...
// source: geon/src/gpu/shaders/apply-torques.wgsl
// helpers-sha256: 165cb4ebb9e35e86beab2561fee3a1f79d834c73f5b65aae6134ec1b4a163356
// wgsl-transpile sha256: 72e2f43a498dc11ed490762511aa70d945310be122342cbfdc6a7e865b1fa84d
...
// wgsl-metrics: {"bytes":23914,"lines":502,"rtVec":0,"rtPoly":0,"rtAtomic":0,"rtNumeric":0,"fround":0,"hypot":0,"iife":0,"workgroupReductionInits":0,"flatWorkgroupArrays":0,"flatWorkgroupSlots":0,"staticBranchPrunes":0}
// generated: 2026-05-30T22:40:33.630Z
export default function _wgsl_module(rt) {
        ...
        if (Gy === 1 && Gz === 1) {
            for (let __gx = Ox; __gx < Xn; __gx++) {
                ...
                __invocation: {
                    ...
                    let _inl_24_result;
                    _inl_24: {
                        _inl_24_result = (((_u_uniforms_toggles0 & GRAVITOMAG_BIT)) != 0);
                        break _inl_24;
                    }
                    const hasGM = _inl_24_result;
                    let _inl_25_result;
                    _inl_25: {
                        _inl_25_result = (((_u_uniforms_toggles0 & GRAVITY_BIT)) != 0);
                        break _inl_25;
                    }
                    const hasGrav = _inl_25_result;
                    let _inl_26_result;
                    _inl_26: {
                    ...
                    const _sroa_0_base = ((idx) * 40 + 24);
                    const t_x = _b_allForces[_sroa_0_base + 0];
                    const t_y = _b_allForces[_sroa_0_base + 1];
                    const t_z = _b_allForces[_sroa_0_base + 2];
                    const t_w = _b_allForces[_sroa_0_base + 3];
                    let torque = 0.0;
                    if ((hasGM && relOn)) {
                        torque = (torque + t_y);
                    }
                    ...
                    const rSq = _b_derived[((idx) * 8 + 7)];
                    const I = ((INERTIA_K * m) * rSq);
                    if ((I < EPSILON)) {
                        break __invocation;
                    }
                    let aw = _b_particles[((idx) * 9 + 6)];
                    aw = (aw + ((torque * _u_uniforms_dt) / I));
                    if ((aw != aw)) {
                        aw = 0.0;
                    }
                    const newAngVel = (relOn ? (aw / Math.sqrt((1.0 + ((aw * aw) * rSq)))) : aw);
                    {
                        const _wbase = ((idx) * 9 + 6) - 6;
                        _b_particles[_wbase + 6] = aw;
                    }
                    const _sroa_1_base = ((idx) * 8);
                    let d_magMoment = _b_derived[_sroa_1_base + 0];
                    let d_angMomentum = _b_derived[_sroa_1_base + 1];
                    let d_invMass = _b_derived[_sroa_1_base + 2];
                    let d_radiusSq = _b_derived[_sroa_1_base + 3];
                    let d_velX = _b_derived[_sroa_1_base + 4];
                    let d_velY = _b_derived[_sroa_1_base + 5];
                    let d_angVel = _b_derived[_sroa_1_base + 6];
                    let d_bodyRSq = _b_derived[_sroa_1_base + 7];
                    d_angVel = newAngVel;
                    {
                        const _wbase = ((idx) * 8);
            ...
            if (Ox === 0 && Oy === 0) {
                for (let __gy = 0, __rowBase = 0; __gy < Gy; __gy++, __rowBase += Gx) {
                    for (let __gx = 0; __gx < Gx; __gx++) {
                        ...
                        __invocation: {
                            ...
                            const relOn = _inl_26_result;
                            const _sroa_2_base = ((idx) * 40 + 24);
                            const t_x = _b_allForces[_sroa_2_base + 0];
                            const t_y = _b_allForces[_sroa_2_base + 1];
                            const t_z = _b_allForces[_sroa_2_base + 2];
                            const t_w = _b_allForces[_sroa_2_base + 3];
                            let torque = 0.0;
                            if ((hasGM && relOn)) {
                                torque = (torque + t_y);
                            }
                            ...
                            let d_magMoment = _b_derived[_sroa_3_base + 0];
                            let d_angMomentum = _b_derived[_sroa_3_base + 1];
                            let d_invMass = _b_derived[_sroa_3_base + 2];
                            let d_radiusSq = _b_derived[_sroa_3_base + 3];
                            let d_velX = _b_derived[_sroa_3_base + 4];
                            let d_velY = _b_derived[_sroa_3_base + 5];
                            let d_angVel = _b_derived[_sroa_3_base + 6];
                            let d_bodyRSq = _b_derived[_sroa_3_base + 7];
                            d_angVel = newAngVel;
                            {
                                const _wbase = ((idx) * 8);
                                _b_derived[_wbase + 0] = d_magMoment;
                ...
                for (let __gx = Ox; __gx < Xn; __gx++) {
                    ...
                    __invocation: {
                        ...
                        const relOn = _inl_26_result;
                        const _sroa_4_base = ((idx) * 40 + 24);
                        const t_x = _b_allForces[_sroa_4_base + 0];
                        const t_y = _b_allForces[_sroa_4_base + 1];
                        const t_z = _b_allForces[_sroa_4_base + 2];
                        const t_w = _b_allForces[_sroa_4_base + 3];
                        let torque = 0.0;
                        if ((hasGM && relOn)) {
                            torque = (torque + t_y);
                        }
                        ...
                        let d_magMoment = _b_derived[_sroa_5_base + 0];
                        let d_angMomentum = _b_derived[_sroa_5_base + 1];
                        let d_invMass = _b_derived[_sroa_5_base + 2];
                        let d_radiusSq = _b_derived[_sroa_5_base + 3];
                        let d_velX = _b_derived[_sroa_5_base + 4];
                        let d_velY = _b_derived[_sroa_5_base + 5];
                        let d_angVel = _b_derived[_sroa_5_base + 6];
                        let d_bodyRSq = _b_derived[_sroa_5_base + 7];
                        d_angVel = newAngVel;
                        {
                            const _wbase = ((idx) * 8);
                            _b_derived[_wbase + 0] = d_magMoment;
            ...
            for (let __gx = Ox; __gx < Xn; __gx++) {
                ...
                __invocation: {
                    ...
                    const relOn = _inl_26_result;
                    const _sroa_6_base = ((idx) * 40 + 24);
                    const t_x = _b_allForces[_sroa_6_base + 0];
                    const t_y = _b_allForces[_sroa_6_base + 1];
                    const t_z = _b_allForces[_sroa_6_base + 2];
                    const t_w = _b_allForces[_sroa_6_base + 3];
                    let torque = 0.0;
                    if ((hasGM && relOn)) {
                        torque = (torque + t_y);
                    }
    ...
    return { entry, bind, bindings: ["uniforms","particles","allForces","derived"], entryInfo };
}
