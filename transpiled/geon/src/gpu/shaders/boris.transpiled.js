...
// source: geon/src/gpu/shaders/boris.wgsl
// helpers-sha256: 165cb4ebb9e35e86beab2561fee3a1f79d834c73f5b65aae6134ec1b4a163356
// wgsl-transpile sha256: 46266802f9e2d5d48fc6bf2a4bbf68236f75e74d92806d5055106ec7804f742c
...
// wgsl-metrics: {"bytes":47783,"lines":843,"rtVec":0,"rtPoly":0,"rtAtomic":0,"rtNumeric":0,"fround":0,"hypot":0,"iife":0,"workgroupReductionInits":0,"flatWorkgroupArrays":0,"flatWorkgroupSlots":0,"staticBranchPrunes":0}
// generated: 2026-05-30T22:40:33.635Z
        ...
        if (Gy === 1 && Gz === 1) {
            for (let __gx = Ox; __gx < Xn; __gx++) {
                ...
                __invocation: {
                    ...
                    const invG = (relOn ? (1.0 / Math.sqrt(((1.0 + (wx * wx)) + (wy * wy)))) : 1.0);
                    const vx = (wx * invG);
                    const vy = (wy * invG);
                    const _sroa_0_base = ((idx) * 8);
                    let d_magMoment = _b_derived[_sroa_0_base + 0];
                    let d_angMomentum = _b_derived[_sroa_0_base + 1];
                    let d_invMass = _b_derived[_sroa_0_base + 2];
                    let d_radiusSq = _b_derived[_sroa_0_base + 3];
                    let d_velX = _b_derived[_sroa_0_base + 4];
                    let d_velY = _b_derived[_sroa_0_base + 5];
                    let d_angVel = _b_derived[_sroa_0_base + 6];
                    let d_bodyRSq = _b_derived[_sroa_0_base + 7];
                    d_velX = ((vx != vx) ? 0.0 : vx);
                    d_velY = ((vy != vy) ? 0.0 : vy);
                    {
                        const _wbase = ((idx) * 8);
                    ...
                    let newPosX = (_b_particles[((idx) * 9 + 0)] + (vx * _u_uniforms_dt));
                    let newPosY = (_b_particles[((idx) * 9 + 1)] + (vy * _u_uniforms_dt));
                    if ((newPosX != newPosX)) {
                        newPosX = _b_particles[((idx) * 9 + 0)];
                    }
                    if ((newPosY != newPosY)) {
                        newPosY = _b_particles[((idx) * 9 + 1)];
                    }
                    {
                        const _wbase = ((idx) * 9 + 0) - 0;
                        _b_particles[_wbase + 0] = newPosX;
                    }
                    {
                        const _wbase = ((idx) * 9 + 1) - 1;
                        _b_particles[_wbase + 1] = newPosY;
                    }
                    ...
                    const Bz = af_bFields_x;
                    const Bgz = af_bFields_y;
                    let _inl_25_result;
                    _inl_25: {
                        _inl_25_result = (((_u_uniforms_toggles0 & MAGNETIC_BIT)) != 0);
                        break _inl_25;
                    }
                    const hasMag = (_inl_25_result || (_u_uniforms_extBz != 0.0));
                    if (hasMag) {
                        af_f1_x = (af_f1_x + ((q * vy) * Bz));
                        af_f1_y = (af_f1_y - ((q * vx) * Bz));
                    }
                    let _inl_26_result;
                    _inl_26: {
                    ...
                    if (_inl_26_result) {
                        af_f1_z = (af_f1_z + (((4.0 * m) * vy) * Bgz));
                        af_f1_w = (af_f1_w - (((4.0 * m) * vx) * Bgz));
                    }
                    {
                        const _wbase = ((idx) * 40);
            ...
            if (Ox === 0 && Oy === 0) {
                ...
                for (let __gx = Ox; __gx < Xn; __gx++) {
                    ...
                    __invocation: {
                        ...
                        let d_magMoment = _b_derived[_sroa_4_base + 0];
                        let d_angMomentum = _b_derived[_sroa_4_base + 1];
                        let d_invMass = _b_derived[_sroa_4_base + 2];
                        let d_radiusSq = _b_derived[_sroa_4_base + 3];
                        let d_velX = _b_derived[_sroa_4_base + 4];
                        let d_velY = _b_derived[_sroa_4_base + 5];
                        let d_angVel = _b_derived[_sroa_4_base + 6];
                        let d_bodyRSq = _b_derived[_sroa_4_base + 7];
                        d_velX = ((vx != vx) ? 0.0 : vx);
                        d_velY = ((vy != vy) ? 0.0 : vy);
                        {
                            const _wbase = ((idx) * 8);
                        ...
                        let af_f0_x = _b_allForces[_sroa_5_base + 0];
                        let af_f0_y = _b_allForces[_sroa_5_base + 1];
                        let af_f0_z = _b_allForces[_sroa_5_base + 2];
                        let af_f0_w = _b_allForces[_sroa_5_base + 3];
                        let af_f1_x = _b_allForces[_sroa_5_base + 4];
                        let af_f1_y = _b_allForces[_sroa_5_base + 5];
                        let af_f1_z = _b_allForces[_sroa_5_base + 6];
                        let af_f1_w = _b_allForces[_sroa_5_base + 7];
                        let af_f2_x = _b_allForces[_sroa_5_base + 8];
                        let af_f2_y = _b_allForces[_sroa_5_base + 9];
                        let af_f2_z = _b_allForces[_sroa_5_base + 10];
                        let af_f2_w = _b_allForces[_sroa_5_base + 11];
                        let af_f3_x = _b_allForces[_sroa_5_base + 12];
                        let af_f3_y = _b_allForces[_sroa_5_base + 13];
                        let af_f3_z = _b_allForces[_sroa_5_base + 14];
                        let af_f3_w = _b_allForces[_sroa_5_base + 15];
                        let af_f4_x = _b_allForces[_sroa_5_base + 16];
                        let af_f4_y = _b_allForces[_sroa_5_base + 17];
                        let af_f4_z = _b_allForces[_sroa_5_base + 18];
                        let af_f4_w = _b_allForces[_sroa_5_base + 19];
                        let af_f5_x = _b_allForces[_sroa_5_base + 20];
                        let af_f5_y = _b_allForces[_sroa_5_base + 21];
                        let af_f5_z = _b_allForces[_sroa_5_base + 22];
                        let af_f5_w = _b_allForces[_sroa_5_base + 23];
                        let af_torques_x = _b_allForces[_sroa_5_base + 24];
                        let af_torques_y = _b_allForces[_sroa_5_base + 25];
                        let af_torques_z = _b_allForces[_sroa_5_base + 26];
                        let af_torques_w = _b_allForces[_sroa_5_base + 27];
                        let af_bFields_x = _b_allForces[_sroa_5_base + 28];
                        let af_bFields_y = _b_allForces[_sroa_5_base + 29];
                        let af_bFields_z = _b_allForces[_sroa_5_base + 30];
                        let af_bFields_w = _b_allForces[_sroa_5_base + 31];
                        let af_bFieldGrads_x = _b_allForces[_sroa_5_base + 32];
                        let af_bFieldGrads_y = _b_allForces[_sroa_5_base + 33];
                        let af_bFieldGrads_z = _b_allForces[_sroa_5_base + 34];
                        let af_bFieldGrads_w = _b_allForces[_sroa_5_base + 35];
                        let af_totalForce_x = _b_allForces[_sroa_5_base + 36];
                        let af_totalForce_y = _b_allForces[_sroa_5_base + 37];
                        let af_jerk_x = _b_allForces[_sroa_5_base + 38];
                        let af_jerk_y = _b_allForces[_sroa_5_base + 39];
                        const q = _b_particles[((idx) * 9 + 5)];
                        const m = _b_particles[((idx) * 9 + 4)];
                        const Bz = af_bFields_x;
                        const Bgz = af_bFields_y;
            ...
            for (let __gx = Ox; __gx < Xn; __gx++) {
                ...
                __invocation: {
                    ...
                    let d_magMoment = _b_derived[_sroa_6_base + 0];
                    let d_angMomentum = _b_derived[_sroa_6_base + 1];
                    let d_invMass = _b_derived[_sroa_6_base + 2];
                    let d_radiusSq = _b_derived[_sroa_6_base + 3];
                    let d_velX = _b_derived[_sroa_6_base + 4];
                    let d_velY = _b_derived[_sroa_6_base + 5];
                    let d_angVel = _b_derived[_sroa_6_base + 6];
                    let d_bodyRSq = _b_derived[_sroa_6_base + 7];
                    d_velX = ((vx != vx) ? 0.0 : vx);
                    d_velY = ((vy != vy) ? 0.0 : vy);
                    {
                        const _wbase = ((idx) * 8);
