...
// source: geon/src/gpu/shaders/field-fft.wgsl
// helpers-sha256: 165cb4ebb9e35e86beab2561fee3a1f79d834c73f5b65aae6134ec1b4a163356
// wgsl-transpile sha256: d0b0f0acb989170df19762a14b70332c5015e558b9923a6cf8fe9ad04d64448e
...
// wgsl-metrics: {"bytes":26898,"lines":567,"rtVec":0,"rtPoly":0,"rtAtomic":0,"rtNumeric":0,"fround":0,"hypot":0,"iife":0,"workgroupReductionInits":0,"flatWorkgroupArrays":0,"flatWorkgroupSlots":0,"staticBranchPrunes":0}
// generated: 2026-05-30T22:00:38.820Z
export default function _wgsl_module(rt) {
    ...
    entryInfo["fftButterfly"] = {"workgroupSize":[256,1,1],"phases":1,"globalLoop":true,"workgroupMemory":false,"flatWorkgroupArrays":0,"optimizedWorkgroupReductionInits":0};
    function __entry_0_fftButterfly(workgroups, bindings, domain, origin) {
        const [Wx, Wy, Wz] = workgroups;
        const Lx = 256, Ly = 1, Lz = 1;
        const _b_bufA = bindings.bufA;
        const _b_bufB = bindings.bufB;
        const _b_fftParams = bindings.fftParams;
        const _u_fftParams_stageLen = _b_fftParams.stageLen;
        const _u_fftParams_direction = _b_fftParams.direction;
        const _u_fftParams_axis = _b_fftParams.axis;
        const _u_fftParams_gridSize = _b_fftParams.gridSize;
        const _u_fftParams_invN = _b_fftParams.invN;
        const _u_fftParams_isLastStage = _b_fftParams.isLastStage;
        ...
        if (Gy === 1 && Gz === 1) {
            for (let __gx = Ox; __gx < Xn; __gx++) {
                ...
                __invocation: {
                    ...
                    const halfLen = _u_fftParams_stageLen;
                    const fullLen = (halfLen * 2);
                    const groupIdx = (posInLine / fullLen);
                    const posInGroup = (posInLine % fullLen);
                    let srcEvenPos = 0;
                    let srcOddPos = 0;
                    if ((posInGroup < halfLen)) {
                        srcEvenPos = ((groupIdx * halfLen) + posInGroup);
                        srcOddPos = (srcEvenPos + (N / 2));
                    } else {
                    ...
                    let oddLinear = 0;
                    if ((_u_fftParams_axis == 0)) {
                        evenLinear = ((((lineIdx * N) + srcEvenPos)) * 2);
                        oddLinear = ((((lineIdx * N) + srcOddPos)) * 2);
                    } else {
                        evenLinear = ((((srcEvenPos * N) + lineIdx)) * 2);
                        oddLinear = ((((srcOddPos * N) + lineIdx)) * 2);
                    }
                    const eRe = _b_bufA[evenLinear];
                    const eIm = _b_bufA[(evenLinear + 1)];
                    const oRe = _b_bufA[oddLinear];
                    const oIm = _b_bufA[(oddLinear + 1)];
                    const k = (posInGroup % halfLen);
                    const angle = ((((+(_u_fftParams_direction)) * TWO_PI) * (+(k))) / (+(fullLen)));
                    ...
                    if ((_u_fftParams_isLastStage != 0)) {
                        outRe = (outRe * _u_fftParams_invN);
                        outIm = (outIm * _u_fftParams_invN);
                    }
                    let outLinear = 0;
                    if ((_u_fftParams_axis == 0)) {
                        outLinear = ((((lineIdx * N) + posInLine)) * 2);
                    } else {
                        outLinear = ((((posInLine * N) + lineIdx)) * 2);
                    }
                    _b_bufB[outLinear] = outRe;
                    _b_bufB[(outLinear + 1)] = outIm;
                }
            }
        } else if (Gz === 1) {
            if (Ox === 0 && Oy === 0) {
    ...
    entry["fftButterfly"] = function ({ workgroups, bindings, domain, origin }) {
        return __entry_0_fftButterfly(workgroups, bindings, domain, origin);
    };

    entryInfo["complexMultiply"] = {"workgroupSize":[256,1,1],"phases":1,"globalLoop":true,"workgroupMemory":false,"flatWorkgroupArrays":0,"optimizedWorkgroupReductionInits":0};
    function __entry_1_complexMultiply(workgroups, bindings, domain, origin) {
        const [Wx, Wy, Wz] = workgroups;
        const Lx = 256, Ly = 1, Lz = 1;
        const _b_bufA = bindings.bufA;
        const _b_fftParams = bindings.fftParams;
        const _u_fftParams_gridSize = _b_fftParams.gridSize;
        const _b_greenHat = bindings.greenHat;
        const Gx = domain && domain[0] != null ? domain[0] : Wx * Lx;
        const Gy = domain && domain[1] != null ? domain[1] : Wy * Ly;
        const Gz = domain && domain[2] != null ? domain[2] : Wz * Lz;
        const Ox = origin && origin[0] != null ? origin[0] : 0;
        ...
        if (Gy === 1 && Gz === 1) {
            for (let __gx = Ox; __gx < Xn; __gx++) {
                ...
                __invocation: {
                    const idx = gid_x;
                    const N2 = (_u_fftParams_gridSize * _u_fftParams_gridSize);
                    if ((idx >= N2)) {
                        break __invocation;
                    }
                    const i = (idx * 2);
                    const aRe = _b_bufA[i];
                    const aIm = _b_bufA[(i + 1)];
                    const bRe = _b_greenHat[i];
                    const bIm = _b_greenHat[(i + 1)];
                    _b_bufA[i] = ((aRe * bRe) - (aIm * bIm));
                    _b_bufA[(i + 1)] = ((aRe * bIm) + (aIm * bRe));
                }
            }
        } else if (Gz === 1) {
            if (Ox === 0 && Oy === 0) {
    ...
    entry["complexMultiply"] = function ({ workgroups, bindings, domain, origin }) {
        return __entry_1_complexMultiply(workgroups, bindings, domain, origin);
    };

    const bind = function (bindings) {
        const bound = Object.create(null);
        bound["fftButterfly"] = function (workgroups, domain, origin) {
            return __entry_0_fftButterfly(workgroups, bindings, domain, origin);
        };
        bound["complexMultiply"] = function (workgroups, domain, origin) {
            return __entry_1_complexMultiply(workgroups, bindings, domain, origin);
        };
        return bound;
    };

    return { entry, bind, bindings: ["bufA","bufB","fftParams","greenHat"], entryInfo };
}
