...
// source: geon/src/gpu/shaders/history.wgsl
// helpers-sha256: 165cb4ebb9e35e86beab2561fee3a1f79d834c73f5b65aae6134ec1b4a163356
// wgsl-transpile sha256: a395b18e64dda43f1e5521b9f6585ae0a4da4e8d21487a4218fd64d544974ad7
...
// wgsl-metrics: {"bytes":23849,"lines":436,"rtVec":0,"rtPoly":0,"rtAtomic":0,"rtNumeric":0,"fround":0,"hypot":0,"iife":0,"workgroupReductionInits":0,"flatWorkgroupArrays":0,"flatWorkgroupSlots":0,"staticBranchPrunes":0}
// generated: 2026-05-30T22:40:33.707Z
export default function _wgsl_module(rt) {
    ...
    entryInfo["recordHistory"] = {"workgroupSize":[64,1,1],"phases":1,"globalLoop":true,"workgroupMemory":false,"flatWorkgroupArrays":0,"optimizedWorkgroupReductionInits":0};
    function __entry_0_recordHistory(workgroups, bindings, domain, origin) {
        const [Wx, Wy, Wz] = workgroups;
        const Lx = 64, Ly = 1, Lz = 1;
        const _b_u = bindings.u;
        const _u_u_simTime = _b_u.simTime;
        const _b_particles = bindings.particles;
        const _b_histData = bindings.histData;
        const _b_histMeta = bindings.histMeta;
        ...
        if (Gy === 1 && Gz === 1) {
            for (let __gx = Ox; __gx < Xn; __gx++) {
                ...
                __invocation: {
                    const i = gid_x;
                    if ((i >= Math.floor(_b_particles.length / 9))) {
                        break __invocation;
                    }
                    const flags = _b_particles[((i) * 9 + 8)];
                    if (((((flags & FLAG_RETIRED)) != 0) && (((flags & FLAG_ALIVE)) == 0))) {
                        if ((((flags & FLAG_DEATH_HIST)) != 0)) {
                            break __invocation;
                        }
                        {
                            const _wbase = ((i) * 9 + 8) - 8;
                            _b_particles[_wbase + 8] = (flags | FLAG_DEATH_HIST);
                        }
                        const metaBase = (i * HIST_META_STRIDE);
                        let writeIdx = _b_histMeta[metaBase];
                        let count = _b_histMeta[(metaBase + 1)];
                        const sampleBase = (((i * HISTORY_LEN) * HIST_STRIDE) + (((writeIdx & HISTORY_MASK)) * HIST_STRIDE));
                        _b_histData[sampleBase] = _b_particles[((i) * 9 + 0)];
                        _b_histData[(sampleBase + 1)] = _b_particles[((i) * 9 + 1)];
                        const wx = _b_particles[((i) * 9 + 2)];
                        const wy = _b_particles[((i) * 9 + 3)];
                        const gamma = Math.sqrt(((1.0 + (wx * wx)) + (wy * wy)));
                        const invG = (1.0 / gamma);
                        _b_histData[(sampleBase + 2)] = (wx * invG);
                        _b_histData[(sampleBase + 3)] = (wy * invG);
                        _b_histData[(sampleBase + 4)] = _b_particles[((i) * 9 + 6)];
                        _b_histData[(sampleBase + 5)] = _u_u_simTime;
                        _b_histMeta[metaBase] = (((writeIdx + 1)) & HISTORY_MASK);
                        _b_histMeta[(metaBase + 1)] = ((HISTORY_LEN) < ((count + 1)) ? (HISTORY_LEN) : ((count + 1)));
                        break __invocation;
                    }
                    ...
                    const metaBase = (i * HIST_META_STRIDE);
                    if ((((flags & FLAG_REBORN)) != 0)) {
                        _b_histMeta[metaBase] = 0;
                        _b_histMeta[(metaBase + 1)] = 0;
                        _b_histMeta[(metaBase + 2)] = rt.bitcast_u32_f32(_u_u_simTime);
                        _b_histMeta[(metaBase + 3)] = 0;
                        {
                            const _wbase = ((i) * 9 + 8) - 8;
                            _b_particles[_wbase + 8] = (flags & (~FLAG_REBORN));
                        }
                    }
                    let writeIdx = _b_histMeta[metaBase];
                    let count = _b_histMeta[(metaBase + 1)];
                    ...
                    _b_histData[(sampleBase + 4)] = _b_particles[((i) * 9 + 6)];
                    _b_histData[(sampleBase + 5)] = _u_u_simTime;
                    writeIdx = (((writeIdx + 1)) & HISTORY_MASK);
                    count = ((HISTORY_LEN) < ((count + 1)) ? (HISTORY_LEN) : ((count + 1)));
                    _b_histMeta[metaBase] = writeIdx;
                    _b_histMeta[(metaBase + 1)] = count;
                }
            }
        } else if (Gz === 1) {
            if (Ox === 0 && Oy === 0) {
    ...
    entry["recordHistory"] = function ({ workgroups, bindings, domain, origin }) {
        return __entry_0_recordHistory(workgroups, bindings, domain, origin);
    };

    const bind = function (bindings) {
        const bound = Object.create(null);
        bound["recordHistory"] = function (workgroups, domain, origin) {
            return __entry_0_recordHistory(workgroups, bindings, domain, origin);
        };
        return bound;
    };

    return { entry, bind, bindings: ["u","particles","histData","histMeta"], entryInfo };
}
