...
// source: geon/src/gpu/shaders/expansion.wgsl
// helpers-sha256: 165cb4ebb9e35e86beab2561fee3a1f79d834c73f5b65aae6134ec1b4a163356
// wgsl-transpile sha256: 05ca11093290f060e7e4372e1b3093d5392f4b49fc0ebc5b1b7cec62f49b78d7
...
// wgsl-metrics: {"bytes":16352,"lines":338,"rtVec":0,"rtPoly":0,"rtAtomic":0,"rtNumeric":0,"fround":0,"hypot":0,"iife":0,"workgroupReductionInits":0,"flatWorkgroupArrays":0,"flatWorkgroupSlots":0,"staticBranchPrunes":0}
// generated: 2026-05-30T22:00:38.804Z
export default function _wgsl_module(rt) {
    ...
    entryInfo["applyExpansion"] = {"workgroupSize":[256,1,1],"phases":1,"globalLoop":true,"workgroupMemory":false,"flatWorkgroupArrays":0,"optimizedWorkgroupReductionInits":0};
    function __entry_0_applyExpansion(workgroups, bindings, domain, origin) {
        const [Wx, Wy, Wz] = workgroups;
        const Lx = 256, Ly = 1, Lz = 1;
        const _b_particles = bindings.particles;
        const _b_eu = bindings.eu;
        const _u_eu_hubbleParam = _b_eu.hubbleParam;
        const _u_eu_dt = _b_eu.dt;
        const _u_eu_centerX = _b_eu.centerX;
        const _u_eu_centerY = _b_eu.centerY;
        const _u_eu_particleCount = _b_eu.particleCount;
        ...
        if (Gy === 1 && Gz === 1) {
            for (let __gx = Ox; __gx < Xn; __gx++) {
                ...
                __invocation: {
                    ...
                    const H = _u_eu_hubbleParam;
                    const dt = _u_eu_dt;
                    p_posX = (p_posX + ((H * ((p_posX - _u_eu_centerX))) * dt));
                    p_posY = (p_posY + ((H * ((p_posY - _u_eu_centerY))) * dt));
                    const decay = (((1.0 - (H * dt))) < (0.0) ? (0.0) : ((1.0 - (H * dt))));
                    p_velWX = (p_velWX * decay);
                    p_velWY = (p_velWY * decay);
                    {
                        const _wbase = ((pid) * 9);
            ...
            if (Ox === 0 && Oy === 0) {
                for (let __gy = 0, __rowBase = 0; __gy < Gy; __gy++, __rowBase += Gx) {
                    for (let __gx = 0; __gx < Gx; __gx++) {
                        ...
                        __invocation: {
                            ...
                            let p_posX = _b_particles[_sroa_1_base + 0];
                            let p_posY = _b_particles[_sroa_1_base + 1];
                            let p_velWX = _b_particles[_sroa_1_base + 2];
                            let p_velWY = _b_particles[_sroa_1_base + 3];
                            let p_mass = _b_particles[_sroa_1_base + 4];
                            let p_charge = _b_particles[_sroa_1_base + 5];
                            let p_angW = _b_particles[_sroa_1_base + 6];
                            let p_baseMass = _b_particles[_sroa_1_base + 7];
                            let p_flags = _b_particles[_sroa_1_base + 8];
                            const flag = p_flags;
                            if ((((flag & FLAG_ALIVE)) == 0)) {
                                break __invocation;
                            }
                ...
                for (let __gx = Ox; __gx < Xn; __gx++) {
                    ...
                    __invocation: {
                        ...
                        let p_posX = _b_particles[_sroa_2_base + 0];
                        let p_posY = _b_particles[_sroa_2_base + 1];
                        let p_velWX = _b_particles[_sroa_2_base + 2];
                        let p_velWY = _b_particles[_sroa_2_base + 3];
                        let p_mass = _b_particles[_sroa_2_base + 4];
                        let p_charge = _b_particles[_sroa_2_base + 5];
                        let p_angW = _b_particles[_sroa_2_base + 6];
                        let p_baseMass = _b_particles[_sroa_2_base + 7];
                        let p_flags = _b_particles[_sroa_2_base + 8];
                        const flag = p_flags;
                        if ((((flag & FLAG_ALIVE)) == 0)) {
                            break __invocation;
                        }
    ...
    entry["applyExpansion"] = function ({ workgroups, bindings, domain, origin }) {
        return __entry_0_applyExpansion(workgroups, bindings, domain, origin);
    };

    const bind = function (bindings) {
        const bound = Object.create(null);
        bound["applyExpansion"] = function (workgroups, domain, origin) {
            return __entry_0_applyExpansion(workgroups, bindings, domain, origin);
        };
        return bound;
    };

    return { entry, bind, bindings: ["particles","eu"], entryInfo };
}
