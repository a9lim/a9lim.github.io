...
// source: geon/src/gpu/shaders/save-f1pn.wgsl
// helpers-sha256: 165cb4ebb9e35e86beab2561fee3a1f79d834c73f5b65aae6134ec1b4a163356
// wgsl-transpile sha256: 5df14ef32a04a65a4c448c823c37c8d08217d9bb5faa7b9f83b3c8515637483d
...
// wgsl-metrics: {"bytes":10760,"lines":244,"rtVec":0,"rtPoly":0,"rtAtomic":0,"rtNumeric":0,"fround":0,"hypot":0,"iife":0,"workgroupReductionInits":0,"flatWorkgroupArrays":0,"flatWorkgroupSlots":0,"staticBranchPrunes":0}
// generated: 2026-05-30T22:40:33.729Z
export default function _wgsl_module(rt) {
        ...
        if (Gy === 1 && Gz === 1) {
            for (let __gx = Ox; __gx < Xn; __gx++) {
                ...
                __invocation: {
                    ...
                    const _sroa_0_base = ((idx) * 40 + 8);
                    const f1pn_x = _b_allForces[_sroa_0_base + 0];
                    const f1pn_y = _b_allForces[_sroa_0_base + 1];
                    const f1pn_z = _b_allForces[_sroa_0_base + 2];
                    const f1pn_w = _b_allForces[_sroa_0_base + 3];
                    _b_f1pnOld[(idx * 2)] = f1pn_x;
                    _b_f1pnOld[((idx * 2) + 1)] = f1pn_y;
                }
            }
        } else if (Gz === 1) {
            if (Ox === 0 && Oy === 0) {
                for (let __gy = 0, __rowBase = 0; __gy < Gy; __gy++, __rowBase += Gx) {
                    for (let __gx = 0; __gx < Gx; __gx++) {
                        ...
                        __invocation: {
                            ...
                            const _sroa_1_base = ((idx) * 40 + 8);
                            const f1pn_x = _b_allForces[_sroa_1_base + 0];
                            const f1pn_y = _b_allForces[_sroa_1_base + 1];
                            const f1pn_z = _b_allForces[_sroa_1_base + 2];
                            const f1pn_w = _b_allForces[_sroa_1_base + 3];
                            _b_f1pnOld[(idx * 2)] = f1pn_x;
                            _b_f1pnOld[((idx * 2) + 1)] = f1pn_y;
                        }
                    }
                ...
                for (let __gx = Ox; __gx < Xn; __gx++) {
                    ...
                    __invocation: {
                        ...
                        const _sroa_2_base = ((idx) * 40 + 8);
                        const f1pn_x = _b_allForces[_sroa_2_base + 0];
                        const f1pn_y = _b_allForces[_sroa_2_base + 1];
                        const f1pn_z = _b_allForces[_sroa_2_base + 2];
                        const f1pn_w = _b_allForces[_sroa_2_base + 3];
                        _b_f1pnOld[(idx * 2)] = f1pn_x;
                        _b_f1pnOld[((idx * 2) + 1)] = f1pn_y;
                    }
                }
            ...
            for (let __gx = Ox; __gx < Xn; __gx++) {
                ...
                __invocation: {
                    ...
                    const _sroa_3_base = ((idx) * 40 + 8);
                    const f1pn_x = _b_allForces[_sroa_3_base + 0];
                    const f1pn_y = _b_allForces[_sroa_3_base + 1];
                    const f1pn_z = _b_allForces[_sroa_3_base + 2];
                    const f1pn_w = _b_allForces[_sroa_3_base + 3];
                    _b_f1pnOld[(idx * 2)] = f1pn_x;
                    _b_f1pnOld[((idx * 2) + 1)] = f1pn_y;
                }
            }
    ...
    return { entry, bind, bindings: ["uniforms","allForces","f1pnOld","particles"], entryInfo };
}
