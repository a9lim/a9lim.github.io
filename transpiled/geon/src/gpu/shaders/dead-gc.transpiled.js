...
// source: geon/src/gpu/shaders/dead-gc.wgsl
// helpers-sha256: 165cb4ebb9e35e86beab2561fee3a1f79d834c73f5b65aae6134ec1b4a163356
// wgsl-transpile sha256: 44ba8601a210dec43a592256d6bf0a4edf007c48de2d651cbede3927f5a4ae3f
...
// wgsl-metrics: {"bytes":19118,"lines":380,"rtVec":0,"rtPoly":0,"rtAtomic":0,"rtNumeric":0,"fround":0,"hypot":0,"iife":4,"workgroupReductionInits":0,"flatWorkgroupArrays":0,"flatWorkgroupSlots":0,"staticBranchPrunes":0}
// generated: 2026-05-30T21:32:08.848Z
export default function _wgsl_module(rt) {
    ...
    function __entry_0_main(workgroups, bindings, domain, origin) {
        ...
        const _b_freeStack = bindings.freeStack;
        const _b_freeTop = bindings.freeTop;
        ...
        if (Gy === 1 && Gz === 1) {
            for (let __gx = Ox; __gx < Xn; __gx++) {
                ...
                __invocation: {
                    const idx = gid_x;
                    const maxSlots = _u_uniforms_aliveCount;
                    if ((idx >= maxSlots)) {
                        break __invocation;
                    }
                    ...
                    const isRetired = (((ps_flags & FLAG_RETIRED)) != 0);
                    if (isRetired) {
                        const deathT = _b_particleAux[((idx) * 5 + 2)];
                        if ((deathT < 1e30)) {
                            const domainDiag = Math.sqrt(((_u_uniforms_domainW * _u_uniforms_domainW) + (_u_uniforms_domainH * _u_uniforms_domainH)));
                            const expiry = (2.0 * domainDiag);
                            if (((_u_uniforms_simTime - deathT) <= expiry)) {
                                break __invocation;
                            }
                        }
                    }
                    ...
                    const slot = (((_r, _k, _v) => { const _o = _r[_k]; _r[_k] = _o + _v; return _o; })(_b_freeTop, 0, 1));
                    _b_freeStack[slot] = idx;
                }
            }
        } else if (Gz === 1) {
            if (Ox === 0 && Oy === 0) {
                for (let __gy = 0, __rowBase = 0; __gy < Gy; __gy++, __rowBase += Gx) {
                    for (let __gx = 0; __gx < Gx; __gx++) {
                        ...
                        __invocation: {
                            ...
                            let ps_posX = _b_particleState[_sroa_1_base + 0];
                            let ps_posY = _b_particleState[_sroa_1_base + 1];
                            let ps_velWX = _b_particleState[_sroa_1_base + 2];
                            let ps_velWY = _b_particleState[_sroa_1_base + 3];
                            let ps_mass = _b_particleState[_sroa_1_base + 4];
                            let ps_charge = _b_particleState[_sroa_1_base + 5];
                            let ps_angW = _b_particleState[_sroa_1_base + 6];
                            let ps_baseMass = _b_particleState[_sroa_1_base + 7];
                            let ps_flags = _b_particleState[_sroa_1_base + 8];
                            if ((((ps_flags & FLAG_ALIVE)) != 0)) {
                                break __invocation;
                            }
                            if ((ps_flags == 0)) {
                ...
                for (let __gx = Ox; __gx < Xn; __gx++) {
                    ...
                    __invocation: {
                        ...
                        const _sroa_2_base = ((idx) * 9);
                        let ps_posX = _b_particleState[_sroa_2_base + 0];
                        let ps_posY = _b_particleState[_sroa_2_base + 1];
                        let ps_velWX = _b_particleState[_sroa_2_base + 2];
                        let ps_velWY = _b_particleState[_sroa_2_base + 3];
                        let ps_mass = _b_particleState[_sroa_2_base + 4];
                        let ps_charge = _b_particleState[_sroa_2_base + 5];
                        let ps_angW = _b_particleState[_sroa_2_base + 6];
                        let ps_baseMass = _b_particleState[_sroa_2_base + 7];
                        let ps_flags = _b_particleState[_sroa_2_base + 8];
                        if ((((ps_flags & FLAG_ALIVE)) != 0)) {
                            break __invocation;
                        }
                        if ((ps_flags == 0)) {
    ...
    return { entry, bind, bindings: ["particleState","particleAux","uniforms","freeStack","freeTop"], entryInfo };
}
