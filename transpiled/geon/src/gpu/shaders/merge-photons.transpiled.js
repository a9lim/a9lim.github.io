...
// source: geon/src/gpu/shaders/merge-photons.wgsl
// helpers-sha256: 165cb4ebb9e35e86beab2561fee3a1f79d834c73f5b65aae6134ec1b4a163356
// wgsl-transpile sha256: 0d6a0978c69af2ec7359ae12aeb4269b4cb1df14660596957c02e99ed0b838ff
...
// wgsl-metrics: {"bytes":22937,"lines":449,"rtVec":0,"rtPoly":0,"rtAtomic":0,"rtNumeric":0,"fround":0,"hypot":0,"iife":2,"workgroupReductionInits":0,"flatWorkgroupArrays":0,"flatWorkgroupSlots":0,"staticBranchPrunes":0}
// generated: 2026-05-30T21:32:08.884Z
export default function _wgsl_module(rt) {
    ...
    const MERGE_ANNIHILATION = 0;

    function appendPhoton(x, y, dirX, dirY, energy) {
        const phIdx = (((_r, _k, _v) => { const _o = _r[_k]; _r[_k] = _o + _v; return _o; })(bindings.phCount, 0, 1));
        if ((phIdx < MAX_PHOTONS)) {
            ...
            let ph_lifetime = 0;
            let ph_flags = 0;
            ph_posX = (x + (dirX * SPAWN_OFFSET_FLOOR));
            ph_posY = (y + (dirY * SPAWN_OFFSET_FLOOR));
            ph_velX = dirX;
            ph_velY = dirY;
            ph_energy = energy;
            ...
            ph_flags = 1;
            {
                ...
                bindings.photons[_wbase + 0] = ph_posX;
                bindings.photons[_wbase + 1] = ph_posY;
                bindings.photons[_wbase + 2] = ph_velX;
                bindings.photons[_wbase + 3] = ph_velY;
                bindings.photons[_wbase + 4] = ph_energy;
                bindings.photons[_wbase + 5] = ph_emitterId;
                bindings.photons[_wbase + 6] = ph_lifetime;
                bindings.photons[_wbase + 7] = ph_flags;
            }
        } else {
            (((_r, _k, _v) => { const _o = _r[_k]; _r[_k] = _o - _v; return _o; })(bindings.phCount, 0, 1));
        }
    }
    ...
    function __entry_0_main(workgroups, bindings, domain, origin) {
        ...
        const _b_mergeResults = bindings.mergeResults;
        const _b_mergeCounter = bindings.mergeCounter;
        ...
        if (Gy === 1 && Gz === 1) {
            for (let __gx = Ox; __gx < Xn; __gx++) {
                ...
                __invocation: {
                    const i = gid_x;
                    if ((i >= ((MAX_PARTICLES) < (_b_mergeCounter[0]) ? (MAX_PARTICLES) : (_b_mergeCounter[0])))) {
                        break __invocation;
                    }
                    const _sroa_0_base = ((i) * 8);
                    const ev_p0_x = _b_mergeResults[_sroa_0_base + 0];
                    const ev_p0_y = _b_mergeResults[_sroa_0_base + 1];
                    const ev_p0_z = _b_mergeResults[_sroa_0_base + 2];
                    const ev_p0_w = _b_mergeResults[_sroa_0_base + 3];
                    const ev_p1_x = _b_mergeResults[_sroa_0_base + 4];
                    const ev_p1_y = _b_mergeResults[_sroa_0_base + 5];
                    const ev_p1_z = _b_mergeResults[_sroa_0_base + 6];
                    const ev_p1_w = _b_mergeResults[_sroa_0_base + 7];
                    if (((((ev_p0_w + 0.5)) >>> 0) != MERGE_ANNIHILATION)) {
                        break __invocation;
                    }
                    const energy = ev_p0_z;
                    if ((energy <= EPSILON)) {
                        break __invocation;
                    }
                    let betaX = (ev_p1_x / energy);
                    let betaY = (ev_p1_y / energy);
                    ...
                    if ((betaSq > EPSILON)) {
                        ...
                        const _inl_24_seed = (((i + 1)) * 747796405);
                        let _inl_24_result;
                        _inl_24: {
                        ...
                        const angle = (_inl_24_result * TWO_PI);
                        perpX = Math.cos(angle);
                        perpY = Math.sin(angle);
                    }
                    const spread = Math.sqrt(((0.0) < ((1.0 - betaSq)) ? ((1.0 - betaSq)) : (0.0)));
                    const ePh = (0.5 * energy);
                    appendPhoton(ev_p0_x, ev_p0_y, (betaX + (spread * perpX)), (betaY + (spread * perpY)), ePh);
                    appendPhoton(ev_p0_x, ev_p0_y, (betaX - (spread * perpX)), (betaY - (spread * perpY)), ePh);
                }
            }
        } else if (Gz === 1) {
            if (Ox === 0 && Oy === 0) {
                for (let __gy = 0, __rowBase = 0; __gy < Gy; __gy++, __rowBase += Gx) {
                    for (let __gx = 0; __gx < Gx; __gx++) {
                        ...
                        __invocation: {
                            ...
                            const ev_p0_x = _b_mergeResults[_sroa_1_base + 0];
                            const ev_p0_y = _b_mergeResults[_sroa_1_base + 1];
                            const ev_p0_z = _b_mergeResults[_sroa_1_base + 2];
                            const ev_p0_w = _b_mergeResults[_sroa_1_base + 3];
                            const ev_p1_x = _b_mergeResults[_sroa_1_base + 4];
                            const ev_p1_y = _b_mergeResults[_sroa_1_base + 5];
                            const ev_p1_z = _b_mergeResults[_sroa_1_base + 6];
                            const ev_p1_w = _b_mergeResults[_sroa_1_base + 7];
                            if (((((ev_p0_w + 0.5)) >>> 0) != MERGE_ANNIHILATION)) {
                                break __invocation;
                            }
                            const energy = ev_p0_z;
                ...
                for (let __gx = Ox; __gx < Xn; __gx++) {
                    ...
                    __invocation: {
                        ...
                        const ev_p0_x = _b_mergeResults[_sroa_2_base + 0];
                        const ev_p0_y = _b_mergeResults[_sroa_2_base + 1];
                        const ev_p0_z = _b_mergeResults[_sroa_2_base + 2];
                        const ev_p0_w = _b_mergeResults[_sroa_2_base + 3];
                        const ev_p1_x = _b_mergeResults[_sroa_2_base + 4];
                        const ev_p1_y = _b_mergeResults[_sroa_2_base + 5];
                        const ev_p1_z = _b_mergeResults[_sroa_2_base + 6];
                        const ev_p1_w = _b_mergeResults[_sroa_2_base + 7];
                        if (((((ev_p0_w + 0.5)) >>> 0) != MERGE_ANNIHILATION)) {
                            break __invocation;
                        }
                        const energy = ev_p0_z;
            ...
            for (let __gx = Ox; __gx < Xn; __gx++) {
                ...
                __invocation: {
                    ...
                    const ev_p0_x = _b_mergeResults[_sroa_3_base + 0];
                    const ev_p0_y = _b_mergeResults[_sroa_3_base + 1];
                    const ev_p0_z = _b_mergeResults[_sroa_3_base + 2];
                    const ev_p0_w = _b_mergeResults[_sroa_3_base + 3];
                    const ev_p1_x = _b_mergeResults[_sroa_3_base + 4];
                    const ev_p1_y = _b_mergeResults[_sroa_3_base + 5];
                    const ev_p1_z = _b_mergeResults[_sroa_3_base + 6];
                    const ev_p1_w = _b_mergeResults[_sroa_3_base + 7];
                    if (((((ev_p0_w + 0.5)) >>> 0) != MERGE_ANNIHILATION)) {
                        break __invocation;
                    }
                    const energy = ev_p0_z;
    ...
    return { entry, bind, bindings: ["mergeResults","mergeCounter","photons","phCount"], entryInfo };
}
