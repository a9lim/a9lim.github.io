...
// wgsl-metrics: {"bytes":75657,"lines":1029,"rtVec":0,"rtPoly":0,"rtAtomic":0,"rtNumeric":0,"fround":0,"hypot":0,"iife":20,"workgroupReductionInits":0,"flatWorkgroupArrays":0,"flatWorkgroupSlots":0,"staticBranchPrunes":0}
// generated: 2026-05-30T21:32:08.791Z
            ...
            if (Oy < __clipYBound) {
                ...
                for (let __gx = Ox; __gx < __clipXn; __gx++) {
                    ...
                    const gid_y = Oy;
                    {
                        ...
                        const u0_blend_x = ((1 * _b_U0_n[((idx_c) * 4 + 0) + 0]) + (0 * _b_U0_other[((idx_c) * 4 + 0) + 0]));
                        const u0_blend_y = ((1 * _b_U0_n[((idx_c) * 4 + 0) + 1]) + (0 * _b_U0_other[((idx_c) * 4 + 0) + 1]));
                        const u0_blend_z = ((1 * _b_U0_n[((idx_c) * 4 + 0) + 2]) + (0 * _b_U0_other[((idx_c) * 4 + 0) + 2]));
                        const u0_blend_w = ((1 * _b_U0_n[((idx_c) * 4 + 0) + 3]) + (0 * _b_U0_other[((idx_c) * 4 + 0) + 3]));
                        const u1_blend_x = ((1 * _b_U1_n[((idx_c) * 4 + 0) + 0]) + (0 * _b_U1_other[((idx_c) * 4 + 0) + 0]));
                        const u1_blend_y = ((1 * _b_U1_n[((idx_c) * 4 + 0) + 1]) + (0 * _b_U1_other[((idx_c) * 4 + 0) + 1]));
                        const u1_blend_z = ((1 * _b_U1_n[((idx_c) * 4 + 0) + 2]) + (0 * _b_U1_other[((idx_c) * 4 + 0) + 2]));
                        const u1_blend_w = ((1 * _b_U1_n[((idx_c) * 4 + 0) + 3]) + (0 * _b_U1_other[((idx_c) * 4 + 0) + 3]));
                        let u0_raw_x = (u0_blend_x + ((1 * dt) * L0_x));
                        let u0_raw_y = (u0_blend_y + ((1 * dt) * L0_y));
                        let u0_raw_z = (u0_blend_z + ((1 * dt) * L0_z));
                        let u0_raw_w = (u0_blend_w + ((1 * dt) * L0_w));
                        let u1_raw_x = (u1_blend_x + ((1 * dt) * L1_x));
                        let u1_raw_y = (u1_blend_y + ((1 * dt) * L1_y));
                        let u1_raw_z = (u1_blend_z + ((1 * dt) * L1_z));
                        let u1_raw_w = (u1_blend_w + ((1 * dt) * L1_w));
