...
export const PROJECTS = [
    {
        ...
        longDesc: 'Activation steering toolkit for HuggingFace transformers. Pulls out contrastive steering vectors and adds them to hidden states at generation time, no fine-tuning needed. Comes with a Python API, an OpenAI-compatible HTTP server, and a terminal UI with live alpha knobs and probe sparklines. Supports 53 model architectures.',
    ...
    },
    {
        ...
        longDesc: 'Fake shell powered by raw LLM completion. Captures the real PS1 at startup and uses it as the stop string; the model hallucinates command output until it emits the next prompt. Seeds from tmux scrollback or shell history, reuses the KV cache across turns, and supports bitsandbytes 4/8-bit quantization on CUDA.',
    ...
    },
    {
        ...
        longDesc: 'Single-user MCP server with three tools: one to fire a shock and two read-only tools for device state and the ops log. Caps intensity and duration, enforces a token-bucket rate limit, and requires an explicit consent flag to raise the conservative defaults. Code-level ceilings make the caps unreachable by editing config.',
    ...
    },
    {
        ...
        longDesc: 'Gerrymandering sandbox with three parties, eight districts, and six fairness metrics including efficiency gap and partisan symmetry. Includes automated pack-and-crack, a simulated-annealing fair-draw optimizer, Monte Carlo election stress tests, and seeded procedural map generation.',
