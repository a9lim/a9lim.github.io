...
- `robots.txt` has tiered crawl delays: Googlebot (1s), Bingbot (2s), default `*` (5s), and AI scraper bots (10s — GPTBot, OAI-SearchBot, ChatGPT-User, ClaudeBot, Codex-SearchBot, Codex-User, PerplexityBot). Google-Extended, Applebot-Extended, and DuckAssistBot have no delay. Explicitly allows `/llms.txt` and `/llms-full.txt`. Disallows `/posts/` (raw markdown) and `/fonts/`. Update when new AI search bots emerge.
