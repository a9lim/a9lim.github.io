...
User-facing prose (blog posts in `posts/`, the home page's bio prose and "Other things" section, the `/resume` Profile blurb, homepage copy including `home.json` content, contact blurbs) is written in a9lim's voice. When rewriting or drafting this content, invoke the `/writing` skill — it has the full voice rubric, anti-patterns, and before/after examples. Scope excludes sim `about.md` files and the `<details class="edu-content">` sections in each sim's `index.html` (those stay in technical-reference register). Functional resume bullets also stay action-verb-led unless explicitly asked otherwise.
...
Single-page portfolio site. Path-based SPA router (`/`, `/projects`, `/blog`, `/resume`, `/blog/{slug}`). The former `/about` route was folded into `/` — the Worker serves a 308 redirect for inbound links. `_worker.js` routes non-static requests to the correct SPA shell (`index.html` for root routes, `scripture/index.html` for `/scripture/*`) and uses HTMLRewriter for:
...
- `src/markdown.js` is the client-side markdown parser. **Also duplicated** as `mdEsc`/`mdInline`/`renderMarkdown` in `_worker.js` (blog SSR) and `_build.js` (feed generation) — update all three when changing the parser. Beyond the lightweight CommonMark subset the parser supports two custom directives and one extended-image syntax:
...
- `_headers` has a **100-rule limit** (~44 rules used). Adding new path-header pairs requires checking headroom. Early Hints are the largest consumer.
