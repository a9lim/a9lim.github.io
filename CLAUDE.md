...
- **SEO injection**: Per-route `<title>`, `<meta description>`, OG tags, `twitter:title`/`twitter:description`, `article:published_time`/`article:modified_time`/`article:author`/`article:tag` (blog posts), canonical URLs, `BlogPosting` + `BreadcrumbList` JSON-LD (blog posts), `Blog` + `ItemList` JSON-LD (`/blog`), `CollectionPage` + `ItemList` JSON-LD (`/projects`, `/scripture/`), `Person` JSON-LD (`/about`), `Book` + `translationOfWork` JSON-LD (scripture work-level), `Chapter` + `BreadcrumbList` JSON-LD (scripture chapters), and `Quotation` JSON-LD (verse deep links).
...
- Each sim's `index.html` has `twitter:title`, `twitter:description`, and keyword-rich `<title>` tags. Three JSON-LD blocks: `["WebApplication", "LearningResource"]` (with `teaches`, `about` array of Wikidata entities, `inLanguage`, `interactivityType`, `datePublished`, `dateModified`, `codeRepository`, `license`), `FAQPage` (8 questions — domain-specific, not boilerplate), and `BreadcrumbList`. All `about` concepts have Wikidata `@id` URIs. Update `dateModified` and FAQ content when making significant sim changes.
- Each sim's `index.html` has a `<details class="edu-content">` section with 400+ words of educational text and cross-sim links before `</body>`. This is crawlable content for SEO/GEO — keep it factually accurate and substantive.
...
- `manifest.json` — Web App Manifest for PWA with 48/192/512px icons (`purpose: "any"`, not maskable — icons have transparent backgrounds), `lang: "en"`, `id: "/"`, and `categories: ["education", "science"]`. Icons generated by `og/generate.js` from `og/icon.html` (logo.svg in `#e11107` on transparent background).
