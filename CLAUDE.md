...
Single-page portfolio site. Path-based SPA router (`/`, `/projects`, `/blog`, `/about`, `/resume`, `/blog/{slug}`). `_worker.js` routes non-static requests to the correct SPA shell (`index.html` for root routes, `scripture/index.html` for `/scripture/*`) and uses HTMLRewriter for:
