...
(function () {
    ...
    var STRINGS_EN = {
        ...
        'resume.research.saklas.b1.post': ') and adds them to hidden states at generation time, no fine-tuning. Ships 26 in-flight probes for affect, stance, register, and alignment.',
        'resume.research.saklas.b2': 'Python API, an OpenAI- and Ollama-compatible server, and a live terminal UI; supports 55 model architectures. On PyPI under AGPL-3.0 with CI and llama.cpp GGUF interchange.',
        ...
        'resume.proj.ogdoad.tagline': 'Rust · Python',
        'resume.proj.ogdoad.b1': "Clifford algebras and quadratic-form classification over the commutative number systems adjacent to Conway's combinatorial games, laid out as one table by place (Archimedean, p-adic, transfinite, finite) and characteristic. Surreals recover the real Cl(p,q) classification with infinite and infinitesimal metric entries; characteristic-2 legs like the nimbers swap the 8-fold periodicity for the Arf invariant and Brauer–Wall group. Verified Rust core with Python bindings.",
    ...
    var STRINGS_JA = {
        ...
        'resume.research.saklas.b1.link': 'Zou et al. 2023',
        'resume.research.saklas.b1.post': ')、生成時の隠れ状態に加算する。微調整不要。情動・態度・口調・アラインメントを推論中にスコアリングする 26 個のプローブを同梱。',
        'resume.research.saklas.b2': 'Python API、OpenAI・Ollama 互換サーバ、ライブな端末 UI を備え、55 種類のモデルアーキテクチャに対応。AGPL-3.0 で PyPI に公開、CI と llama.cpp の GGUF 入出力対応つき。',
        'resume.research.kaomoji.tagline': 'Python · HuggingFace',
        ...
        'resume.proj.ogdoad.tagline': 'Rust · Python',
        'resume.proj.ogdoad.b1': 'Conway の組合せゲームに隣接する可換な数体系の上で、クリフォード代数と二次形式の分類を行う。位置(アルキメデス的・p 進的・超限的・有限)と標数を軸とする 1 つの表に並べる。超現実数は無限・無限小の計量成分を持つ実 Cl(p,q) の分類を復元し、ニンバーのような標数 2 の系では 8 周期性が Arf 不変量と Brauer–Wall 群に置き換わる。検証済みの Rust コアと Python バインディングを備える。',
        'resume.proj.bioformer.tagline': 'Python',
